...
![네 가지 전형 구조와 그 지표 값](img/fig1_archetypes.png)
...
![컴퓨터(HS 8471) 네트워크, 1995년과 2024년](img/fig2_computers_1995_2024.png)
...
다음 표는 지표마다 계산에 쓴 그래프의 형태와 계산 스크립트를 정리한 것이다. 그래프 열의 "방향"과 "방향 없음"은 엣지 방향을 쓰는지를, "가중"과 "무가중"은 금액을 쓰는지를 뜻한다.

| 지표 | 그래프 | 계산 스크립트 |
|---|---|---|
| 활성 국가 수, 밀도, 호혜성 | 방향, 무가중 | [`scripts/04_compute_metrics.py`](../../scripts/04_compute_metrics.py) |
...
| 역내 비중 | 방향, 가중 | 같음 |
| 대시보드의 노드·엣지 | 방향, 가중, 상위 3개 공급국 | [`scripts/10_export_network_json.py`](../../scripts/10_export_network_json.py) |
| 이 문서의 그림 1·2와 인용 수치 | 위와 같은 정의 | 이 폴더의 [`analysis.ipynb`](analysis.ipynb) |

...
| `analysis/network-theory/` | 해설 문서 「교역 네트워크를 읽는 지표」, 그림 두 장, 그림과 인용 수치를 만드는 노트북 |
| `analysis/china-hub/` | 분석 보고서 「중국 허브화와 세계 교역 네트워크, 1995\~2024」, 그림 여섯 장, 표 수치와 그림을 만드는 노트북 |
...
연구 폴더의 표 수치와 그림은 각 폴더의 `analysis.ipynb`가 만든다(커널 "Python (kcsdb)"). 위에서 아래로 실행하면 그림은 `img/`에, 표는 `outputs/`(저장소 제외)에 저장된다. 보고서 노트북은 마지막 셀에서 보고서가 인용한 수치를 다시 계산해 대조한다.
